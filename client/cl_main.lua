...
RegisterNuiCallback('swapGarage', function(data, cb)
    SetNuiFocus(false, false)
    TriggerServerEvent('ss-garage:server:SwapGarage', data)
    cb('ok')
end)
...
RegisterNuiCallback('payForImpound', function(data, cb)
    SetNuiFocus(false, false)
    TriggerEvent('ss-garage:client:PayForImpound', data)
    cb('ok')
end)
...
function CreateZone(index, garage)
    ...
    if not comboZone then
        comboZone = ComboZone:Create(garageZones, { name = 'garageCombo', debugPoly = false })
    else
                    ...
                    if IsControlJustReleased(0, 38) then
                        ...
                        if not IsPedInAnyVehicle(PlayerPedId(), false) then
                            ...
                            if Config.Garages[CurrentGarage].type ~= 'depot' then
                                ...
                                local garage = zone.data.indexgarage

                                TriggerEvent('ss-garage:client:ParkVehicle', vehicle, plate, vehProps, garage)
                            end
                        end
                    end
                end
...
local function CreateBlip(coords, garage)
    if garage.blip then
...
function CreateBlipsZones()
    for index, garage in pairs(Config.Garages) do
        ...
        if shouldCreate then
            CreateZone(index, garage)
            CreateBlip(garage.coords, garage)
        end
    end
end

RegisterNetEvent('ss-garage:client:CreateBlipsZones', function()
    CreateBlipsZones()
end)
